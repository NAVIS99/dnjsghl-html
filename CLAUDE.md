...
- `{함축 단어}`: 이번 작업을 한 단어(필요하면 하이픈 연결)로 요약 — 예: `overseas-country`,
  `radio-check`, `complete-sticky`, `chip`. 영문 kebab-case 권장(기존 파일들과 통일).
...
- 같은 세션에서 한 작업은 보통 파일 하나에 모은다. 성격이 전혀 다른 작업을 같은 날 여러 건 하면
  주제별로 파일을 나눠도 된다.
- 코드만 바꾸고 이 기록을 빼먹지 않는다 — 빠지면 개발자가 인라인 핸드오프의 변경 의도를 못 따라온다.
