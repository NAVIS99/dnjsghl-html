...
**커밋/푸시는 모아서 한 번에 한다 — 매 수정마다 커밋·푸시하지 않는다(사용자 선호, 2026-06-30).**
작은 변경 하나하나를 곧바로 push하지 말고, 한 작업 단위(또는 사용자가 "이제 올려줘"라고 할 때)가
끝났을 때 여러 변경을 묶어 **한 커밋·한 push**로 처리한다. 작업 도중에는 파일만 수정해 두고, 커밋은
마무리 시점에 한 번에. 사용자가 명시적으로 push를 요청하거나 자연스러운 마무리 지점이 아니면 굳이
push하지 않는다.
